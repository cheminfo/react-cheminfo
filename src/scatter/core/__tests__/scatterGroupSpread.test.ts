...
test('a grid with no axes and no groups measures nothing at all', () => {
  ...
  expect(scatterPairEllipse(spread, 0, 0, 1)).toBeNull();
  expect(scatterSkippedGroups(spread, 0, 1)).toStrictEqual([]);
});

test('the skipped groups are the ones too small to outline, and only those', () => {
  expect(scatterSkippedGroups(measured(), 0, 1)).toStrictEqual([]);
  ...
  expect(scatterSkippedGroups(spread, 0, 1)).toStrictEqual([0]);
  expect(scatterSkippedGroups(spread, 2, 3)).toStrictEqual([0]);
});

test('an axis outside the block skips every group', () => {
  expect(scatterSkippedGroups(measured(), 0, 4)).toStrictEqual([0, 1, 2]);
});
